...
from buildingblocks import SingleConv, Encoder, Decoder, DoubleConv, ExtResNetBlock, FusionBlock, discriminator_block, ZeroLayerF, CBAM, Reshape, VAEUp, MCBAM, SpatialGate, ProductOfExperts, ProductOfExperts2
from utils import number_of_features_per_level

class ReconDecoder(nn.Module):
    'Decoder for reconstruction'
    def __init__(self, basic_module=ExtResNetBlock, multi_stream = 4, f_maps=64, MVAE_reduction=False, ks=3, num_groups=8, num_levels=4, 
                 layer_order='gcr', conv_kernel_size=3, conv_padding=1):
        ...
        if basic_module == DoubleConv and not MVAE_reduction:
        ...
        for m in range(multi_stream):
            ...
            finals.append(nn.Conv3d(f_maps[0], 4, 1))
...
class AbstractFusion3DUNet(nn.Module):
    ...
    Args:
        ...
        MVAE_reduction : reduce a MVAE latent dimension (DAB)
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr', multi_stream = 4, fusion_level = 4,
                 recon_decoder=False, recon_skip=False, MVAE=False, MVAE_reduction=False,
                 num_groups=8, num_levels=4, num_block=[1,1,1,1], conv_kernel_size=3, pool_kernel_size=2, conv_padding=1, **kwargs):
...
#             dec_f_maps = number_of_features_per_level(f_maps // 2, num_levels=num_levels) # small decoder
            dec_f_maps = number_of_features_per_level(f_maps * 2, num_levels=num_levels) # big decoder
        num_block = num_block
        ...
        for i, out_feature_num in enumerate(enc_f_maps):
            ...
            if MVAE_reduction:
                ...
                for j in range(multi_stream):
                    ...
                    level_DABs.append(layers)

                level_VU_layers = nn.Sequential(
                        nn.Conv3d(self.MVAE_latents[i], dec_f_maps[i], 1, 1),
#                         nn.Identity(),
                )
                atten_block = CBAM(dec_f_maps[i])

                DABs.append(nn.ModuleList(level_DABs))
                VU_blocks.append(level_VU_layers)    
                atten_blocks.append(atten_block)
        ...
        if recon_decoder:
            self.rdecoder = ReconDecoder(basic_module=basic_module, multi_stream = multi_stream, f_maps=rec_f_maps, MVAE_reduction=MVAE_reduction,
...
#                     level_features[j] = ZeroLayerF.apply(feature, drop[:, j])
                ...
                if self.MVAE: # multi-level MVAE
                    mod_mu = []
                    mod_logvar = []
                    for j, feature in enumerate(level_features):
                        if self.MVAE_reduction:
                            feature = self.DABs[level][j](feature)
                            pass
...
#                     logvar_list.append(logvar)
                    ...
                    for idx in subset_idx_list:
                        ...
                        if idx in subset_idx_list: # len : 4
                            ...
                            if self.MVAE_reduction:
                                ...
                                x, _ = self.atten_blocks[level]([x]) # atten block
                ...
                x = level_encs(x)
#             encoders_features.insert(0, x) # full x
            encoders_features.insert(0, recon_x_list[0][0]) # first full or missing subset
        ...
        if recon and self.recon_decoder:
            if self.recon_skip:
                recon_x = recon_x_list[0][0]
                recon_features = recon_x_list[0][1:]
                recon_outputs = self.rdecoder(recon_features, recon_x) # list [m1,...,m4]
                ...
                recon_outputs = self.rdecoder(None, bottleneck, size_list)
            if self.MVAE:
